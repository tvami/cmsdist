...
%define tag f4c022373a6d0d97cbfc9588527112a00ec2a34b
%define branch main
...
cp -pr %_builddir/stuff/rtgu src/cpp
...
# Build
%build
PYTHONPATH=${PYTHON27PATH} python setup.py -v build_system -s DQM -d

# Install
%install
mkdir -p %i/{128,}/etc/profile.d %i/128/{x,}{bin,lib,include,data} %i/128/{x,}$PYTHON_LIB_SITE_PACKAGES
PYTHONPATH=${PYTHON27PATH} python setup.py install_system -s DQM --prefix=%i/128
...
%{relocateConfig}/128/etc/profile.d/env.sh
cp $RPM_INSTALL_PREFIX/%{pkgrel}/etc/profile.d/{init,dependencies-setup}.*sh $RPM_INSTALL_PREFIX/%{pkgrel}/128/etc/profile.d/
